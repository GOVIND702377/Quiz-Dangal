...
const PWAInstallButton = () => {
  ...
  // Show nothing if app is installed or neither APK nor PWA install is available
  if (isStandalone || (!hasApk && !canInstall)) return null;

  return (
    <button
      onClick={hasApk ? forceDownload : handleInstall}
      ...
      aria-label={hasApk ? 'Download App' : 'Install App'}
      title={hasApk ? 'Download App' : 'Install App'}
    >
  ...
  <span className="hidden group-hover:block absolute right-16 bg-gray-900 text-white px-2 py-1 rounded text-xs whitespace-nowrap">{hasApk ? 'Download App' : 'Install App'}</span>
    </button>
  );
};
